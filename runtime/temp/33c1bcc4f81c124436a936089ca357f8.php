<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:80:"/www/wwwroot/vorp.top/application/templates/pc/merchant/default/goods/index.html";i:1539744090;s:73:"/www/wwwroot/vorp.top/application/templates/pc/merchant/default/base.html";i:1539744040;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
    <head>
        <meta charset="utf-8">
					...
					<form class="form-inline" role="form" action="" method="get">
						<div class="form-group">
							<select name="cate_id" class="form-control">
								<option value="" <?php if(\think\Request::instance()->get('cate_id')==''): ?>selected<?php endif; ?>>全部分类 </option> <?php foreach($categorys as $v): ?> <option
								 value="<?php echo $v['id']; ?>" <?php if(\think\Request::instance()->get('cate_id')==$v['id']): ?>selected<?php endif; ?>><?php echo $v['name']; ?> </option> <?php endforeach; ?> </select> </div>
								 <div class="form-group">
									<input name="name" type="text" class="form-control" placeholder="商品名" value="<?php echo (\think\Request::instance()->get('name') ?: ''); ?>">
						</div>
						<button type="submit" class="btn btn-purple waves-effect waves-light"><i class="fa fa-search"></i> 搜索</button>
					</form>
				</div>
				<div class="col-sm-6">
					<a href="<?php echo url('add'); ?>" class="pull-right btn btn-purple waves-effect waves-light">添加商品</a>
			...
			<div class="row">
				<table class="table table-striped m-0">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" name="good_id" value="<?php echo $v['id']; ?>"></td>
							<td><?php echo (isset($v['category']['name']) && ($v['category']['name'] !== '')?$v['category']['name']:"未分类"); ?></td>
							<td><?php echo $v['sort']; ?></td>
							<th scope="row"><?php echo $v['name']; if($v['wholesale_discount']==1): ?>
							...
							<td>
								...
								<a href="/merchant/goods_card/index?goods_id=<?php echo $v['id']; ?>&status=1">查看</a>
							...
							<td>
								...
								<a href="/merchant/goods_card/index?goods_id=<?php echo $v['id']; ?>&status=2">查看</a>
								<a href="/merchant/goods/dumpCards?goods_id=<?php echo $v['id']; ?>&status=2" target="_blank">导出</a></td>
							<td>
								<input onchange="change_status(this,<?php echo $v['id']; ?>)" type="checkbox" data-plugin="switchery" data-color="#2bb309"
								 data-size="small" data-switchery="true" style="display: none;" <?php if($v['status']==1): ?>checked<?php endif; ?>> </td> <td><?php echo date("Y-m-d
								H:i:s",$v['create_at']); ?></td>
							<td>
								<a href="<?php echo url('link'); ?>?id=<?php echo $v['id']; ?>">链接</a>
								<span>|</span>
								<a href="<?php echo url('goods_card/add'); ?>?goods_id=<?php echo $v['id']; ?>">加卡</a>
								<span>|</span>
								<a href="<?php echo url('edit'); ?>?id=<?php echo $v['id']; ?>">编辑</a>
								<span>|</span>
